  ...
  public polygon(points: Point[], negative = false): void {
    ...
    this.parts.push(
      `M${round(this.x(first[0]))} ${round(this.y(first[1]))}` +
        rest.map((p) => `L${round(this.x(p[0]))} ${round(this.y(p[1]))}`).join("") +
        "Z",
    );
  }
...
const SVG_HEADER = "<!-- croco calc mark. Generated by scripts/generate-icon.ts. -->";
...
function assets(): void {
  ...
  writePng(path.join(FAVICON_DIR, "apple-touch-icon.png"), square, 180, 180, bg);
  writePng(
  ...
  for (const [w, h] of [
    ...
    writePng(path.join(FAVICON_DIR, `mstile-${w}x${h}.png`), mark, w, h, bg, 0.14);
  }
  writePng(path.join(ICONS_DIR, "general_icon_x512.png"), square, 512, 512, null);
  // Maskable: the plate colour matches the canvas, so the extra padding buys
  // the >= 20 % safe area the spec wants (INF-114).
  writePng(path.join(ICONS_DIR, "maskable_icon_x512.png"), square, 512, 512, bg, 0.16);
  writePng(
...
if (mode === "generate") {
  ...
  console.error("usage: generate-icon.ts <generate [--count N] | svg | assets>");
  process.exit(1);
}
